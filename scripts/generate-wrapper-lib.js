...
const WRAPPED_CACHE_PATH = path.join(__dirname, 'wrapped-mui.json')
const EXCLUDED_CACHE_PATH = path.join(__dirname, 'excluded-mui.json')
const MAX_GENERATE_PASSES = 8
const GENERATED_FILE_BANNER = '// Generated by scripts/generate-wrapper-lib.js — do not edit by hand.\n'
...
function buildComponentFileContent(componentName, importPath, useWrapped) {
  ...
  return `${GENERATED_FILE_BANNER}${coreImport}
...
function renderModuleFiles(config, importEntries, wrappedMuiSet) {
  ...
  fs.writeFileSync(path.join(outDir, 'index.ts'), `${GENERATED_FILE_BANNER}${indexContent}\n`)
...
function runTypeCheck() {
  ...
  return spawnSync(process.execPath, [tscBin, '--noEmit', '--pretty', 'false'], {
    cwd: ROOT,
    encoding: 'utf8',
  })
}
...
function main() {
  ...
  let lastWrappedSnapshot = JSON.stringify(wrappedCache)
  let lastExcludedSnapshot = JSON.stringify(excludedCache)

  for (let i = 0; i < MAX_GENERATE_PASSES; i += 1) {
    ...
    const wrappedChanged = JSON.stringify(nextWrapped) !== lastWrappedSnapshot
    const excludedChanged = JSON.stringify(nextExcluded) !== lastExcludedSnapshot
    if (!wrappedChanged && !excludedChanged) break
    wrappedCache = nextWrapped
    excludedCache = nextExcluded
    lastWrappedSnapshot = JSON.stringify(wrappedCache)
    lastExcludedSnapshot = JSON.stringify(excludedCache)
    if (i === MAX_GENERATE_PASSES - 1) {
      throw new Error(
        `generate:lib did not converge after ${MAX_GENERATE_PASSES} passes. ` +
          'Commit scripts/wrapped-mui.json and scripts/excluded-mui.json after running locally, ' +
          'or extend MAX_GENERATE_PASSES in scripts/generate-wrapper-lib.js.',
      )
    }
  }
  ...
  const finalTypeCheck = runTypeCheck()
  if (finalTypeCheck.status !== 0) {
    const output = `${finalTypeCheck.stdout ?? ''}\n${finalTypeCheck.stderr ?? ''}`.trim()
    throw new Error(`generate:lib finished with TypeScript errors:\n${output}`)
  }
